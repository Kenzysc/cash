...
function testAudioPlayback() {
    // let audioFilePath = cordova.file.cacheDirectory + "www/assets/BabyElephantWalk60.wav"; // Use an actual file path
    // let media = new Media(audioFilePath, function () {
    ...
    document.addEventListener("deviceready", function () {
        // Path to the WAV file in the cache directory
        let audioFilePath = cordova.file.cacheDirectory + "BabyElephantWalk60.wav";

        window.resolveLocalFileSystemURL(audioFilePath, function (fileEntry) {
            // File exists, you can now use fileEntry to interact with the file
            console.log("File found!");

            // For example, you can use the Cordova Media Plugin to play the audio
            var media = new Media(audioFilePath,
                function () { console.log("Audio playback successful!"); },
                function (error) { console.log("Audio playback error: " + error); });
...
// var playAudioCordova = function (sampleView) {
//     // Convert Float32Array to PCM16 (required for most native plugins)
//     let pcmData = convertFloat32ToPCM16(sampleView);

//     // Save the PCM data to a temporary file
//     let audioFilePath = cordova.file.cacheDirectory + "quiet_audio.wav";
//     savePCMToFile(pcmData, audioFilePath, function (success) {
//         // Play the audio file using cordova-plugin-media
//         let media = new Media(audioFilePath, function () {
...
//         media.play();
//     }, function (err) {
//         console.error("Failed to save PCM data:", err);
//     });
// };

function convertFloat32ToPCM16(floatArray) {
    let pcmData = new Int16Array(floatArray.length);
...
function savePCMToFile(pcmData, filePath, onSuccess, onError) {
    window.resolveLocalFileSystemURL(cordova.file.cacheDirectory, function (dir) {
        dir.getFile("quiet_audio.wav", { create: true }, function (file) {
            file.createWriter(function (writer) {
                let blob = new Blob([pcmData.buffer], { type: "audio/wav" });
                writer.write(blob);
                writer.onwriteend = () => {
                    console.log("File saved successfully at: " + file.fullPath);
...
var testPCMConversion = function () {
    let testSamples = new Float32Array([0.5, -0.5, 0.0, 1.0, -1.0]);
    let pcmData = convertFloat32ToPCM16(testSamples);
    console.log("PCM Data: ", pcmData); // Should output Int16Array values like [16384, -16384, 0, 32767, -32768]
};
testPCMConversion();
...
var Quiet = (function () {
    ...
    function transmitter(opts) {
        ...
        var onaudioprocess_prev = function (e) {
        ...
        var onaudioprocess = function (e) {
            if (played === true) {
                // Skip playback if already played
                return;
            }

            played = true;

            alert("Sample View Data: ", sample_view.slice(0, 10)); // Logs first 10 samples

            // Play the current sample view using Cordova
            // playAudioCordova(sample_view);
            testAudioPlayback();
        };
    ...
    function createAudioInput() {
        ...
        //     cordova.plugins.audioinput.initialize(function () {
        //         audioInput = audioCtx.createMediaStreamSource(audioinput.getAudioStream());
        ...
        //     });
        // }, 0); for webview i guess

        document.addEventListener('deviceready', function () {
            cordova.plugins.audioinput.initialize(function () {
                audioInput = cordova.plugins.audioinput.getAudioStream({
                    sampleRate: 44100, // Match the encoding settings
                    bufferSize: 4096,
                    channels: 1,       // Mono
                    format: 'PCM_16BIT'
                });
                ...
                console.error('AudioInput initialization failed:', error);
                audioInputFailed(error);
            });
        }, false);
    };
